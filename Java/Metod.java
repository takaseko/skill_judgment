...
1 メソッドとは
・メソッドとは、クラス内に書かれた処理（いくつかの処理をまとめたもの）のこと
・メソッドには、処理を実行するための条件である「引数」（パラメータ）を渡すことができます。
・そして、処理の実行結果を「戻り値」として返却します。
・※複数の引数を指定する場合は「total(int a, int b）」のようにカンマ区切りで指定します。また引数を指定しない場合は「total()」のように記述。
